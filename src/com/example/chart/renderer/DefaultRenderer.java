...
public class DefaultRenderer implements Serializable {
   ...
   */
	...
	private boolean						mShowXAxes				= true;
	private boolean						mShowYAxes				= true;
	...
	private float						mLabelsTextSize			= 30;
	...
	private boolean						mShowGridX				= true;
	/** If the Y axis grid should be displayed. Y轴网格是否需要显示。 */
	private boolean						mShowGridY				= true;
	...
	private int[]						mMargins				= new int[] { 0, 0, 0, 0 };
	...
	private float						selectableBuffer		= 15;
	/** If the chart should display the values (available for pie chart). */
	private boolean						mDisplayValues;

	/**
	 * A flag to be set if the chart is inside a scroll and doesn't need to
	 ...
	 * 
	 * @param index
	 *            the index in the simple renderers list 指数,该指数在简单的渲染器列表中
	...
	/**
	 * Returns the simple renderers count in the multiple renderer list. 返回
	 * 简单的渲染器系列的List集合的个数。
	 * 
	 ...
	 */
	public void setShowXAxes(boolean showXAxes) {
		mShowXAxes = showXAxes;
	}
	...
	/**
	 * Sets if the X axis grid should be visible. 设置是否显示表格中X的网格线
	 * 
	 * @param showGrid
	...
	/**
	 * Sets if the Y axis grid should be visible. 设置是否显示表格中Y的网格线
	 * 
	 * @param showGrid
	...
	/**
	 * Sets if the grid should be visible. 设置是否显示表格中X、Y的网格线
	 * 
	 * @param showGrid
	...
	/**
	 * Returns if the legend should be visible. 返回 这个说明、图例 是否可见
	 * 
	...
	/**
	 * Returns if the legend should size to fit. 返回 这个说明、图例 字体的大小是否合适
	 * 
	...
	/**
	 * Returns the enabled state of the zoom. 设置X轴和Y轴是否可以被缩放 子类中也有
	 * 
	...
	/**
	 * Returns the visible state of the zoom buttons. 返回 右下角的变焦按钮的可见性
	 * 
	...
	/**
	 * Sets the visible state of the zoom buttons. 设置 右下角的变焦按钮的可见性
	 * 
	 * @param visible
	...
	/**
	 * Returns the zoom rate. 返回 变焦率。(应该是放到或缩小的倍数)
	 * 
	...
	/**
	 * Returns the enabled state of the pan. 返回 画笔授权(是否启用画笔) 如果设置为false的话，则无法拖动
	 * 
	...
	/**
	 * Sets the zoom rate. 设置 变焦率。(应该是放到或缩小的倍数)
	 * 
	 * @param rate
	 *            the zoom rate
	 */
	 ...
	 */
	public float getSelectableBuffer() {
	 ...
	 */
	public void setSelectableBuffer(float buffer) {
	 ...
	 * top, left, bottom, right 返回 距离边缘的尺寸大小 数组顺序:[上、左、下、右]
	 * 
